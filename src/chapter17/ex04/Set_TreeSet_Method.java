...
//			- TreeSet<E>		: 입력 순서대로 출력되지 않고 랜덤하게 값이 출력
//			- LinkedTreeSet<E>	: 입력 순서대로 값이 출력
...
//					- 내림차순 정렬 (DESC)(10,9,...,1)(Z,Y,...,A)(하,파,...가)
		...
		tSet1.add("가"); // "가" 가 중복됨 <== Set은 중복된 값을 저장할수 없다.
		tSet1.add("나");
		tSet1.add("가");
		System.out.println(tSet1); // [가, 나]
		...
		tSet2.add("다");
		tSet2.addAll(tSet1);
		System.out.println(tSet2); // [가, 나, 다]

		// 3. remove (Object o) : 값으로 삭제, (방번호로 삭제 불가)
		tSet2.remove("나");
		System.out.println(tSet2); // [가, 다]
		System.out.println(tSet2.toString()); // 컬랙션의 모든 구현체는 toString()이 재정의 되어있다.
		...
		boolean bool1 = tSet2.isEmpty();
		System.out.println(bool1); // true
		System.out.println(tSet2.isEmpty()); // true
		...
		tSet3.add("나");
		tSet3.add("다");
		System.out.println(tSet3.contains("나")); // true
		System.out.println(tSet3.contains("라")); // false

		// 7. size() : set에 저장된 값의 갯수
		System.out.println(tSet3.size()); // 3
...
//				next() : 값을 출력하고, 다음 값으로 이동
		...
		// 8-1 Iterator 객체 생성
		Iterator<String> iterator = tSet3.iterator(); // set 객체에 iterator를 장창 (import)

		// 8-2 Iterator 객체를 사용해서 tSet3의 저장된 값을 가져온다
		...
		String[] str = tSet3.toArray(new String[0]); // String[] 배열로 꺼낸다.
